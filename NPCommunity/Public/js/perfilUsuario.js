var botonUsuPosts;
var botonUsuGrupos;
var botonUsuProductos;
var rowContenidoPerfil;
var usuarioBuscado;
document.addEventListener("DOMContentLoaded", function () {
  rowContenidoPerfil = document.getElementById("contenidoUsuPerfil");
  botonUsuPosts = document.getElementById("mostrarUsuPosts");
  botonUsuGrupos = document.getElementById("mostrarUsuGrupos");
  botonUsuProductos = document.getElementById("mostrarUsuProductos");
  usuarioBuscado = document.getElementById("nombrePersona");

  botonUsuPosts.addEventListener("click", mostrarUsuPosts);
  botonUsuGrupos.addEventListener("click", mostrarUsuGrupos);
  botonUsuProductos.addEventListener("click", mostrarUsuProductos);
...
function mostrarUsuGrupos() {
  botonUsuGrupos.classList.add("active");
  botonUsuPosts.classList.remove("active");
  botonUsuProductos.classList.remove("active");
  rowContenidoPerfil.innerHTML = "";
  spiner();
  xhr = new XMLHttpRequest();
  xhr.addEventListener("readystatechange", gestionarRespuestaUsuGrupos, true);
  xhr.open("GET", "/usuGrupos/" + usuarioBuscado.innerHTML);
  xhr.send();
}

function gestionarRespuestaUsuGrupos(evento) {
  if (evento.target.readyState == 4 && evento.target.status == 200) {
    ...
    if (objetoRespuesta.length > 0) {
      ...
      columna.innerHTML =
        ...
        usuarioBuscado.innerHTML +
        " no ha creado ningún grupo</h1>";
      rowContenidoPerfil.appendChild(columna);
    }
  }
}
...
function mostrarUsuProductos() {
  botonUsuProductos.classList.add("active");
  botonUsuPosts.classList.remove("active");
  botonUsuGrupos.classList.remove("active");
  rowContenidoPerfil.innerHTML = "";
  spiner();
  xhr = new XMLHttpRequest();
  xhr.addEventListener(
    "readystatechange",
    gestionarRespuestaUsuProductos,
    true
  );
  xhr.open("GET", "/usuProductos/" + usuarioBuscado.innerHTML);
  xhr.send();
}

function gestionarRespuestaUsuProductos(evento) {
  if (evento.target.readyState == 4 && evento.target.status == 200) {
    rowContenidoPerfil.innerHTML = "";
    objetoRespuesta = JSON.parse(evento.target.responseText);
    if (objetoRespuesta.length > 0) {
      ...
      columna.innerHTML =
        "<h1 style='text-aling:center'>@" +
        usuarioBuscado.innerHTML +
        " no ha publicado ningún producto</h1>";
      rowContenidoPerfil.appendChild(columna);
    }
  }
}

function mostrarUsuPosts() {
  botonUsuGrupos.classList.remove("active");
  botonUsuProductos.classList.remove("active");
  botonUsuPosts.classList.add("active");
  rowContenidoPerfil.innerHTML = "";
  spiner();
  xhr = new XMLHttpRequest();
  xhr.addEventListener("readystatechange", gestionarRespuestaUsuPosts, true);
  xhr.open("GET", "/usuPosts/" + usuarioBuscado.innerHTML);
  xhr.send();
}

function gestionarRespuestaUsuPosts(evento) {
  if (evento.target.readyState == 4 && evento.target.status == 200) {
    rowContenidoPerfil.innerHTML = "";
    objetoRespuesta = JSON.parse(evento.target.responseText);
    if (objetoRespuesta.length > 0) {
      ...
      columna.innerHTML =
        "<h1 style='text-aling:center'>@" +
        usuarioBuscado.innerHTML +
        " no ha publicado ningún post</h1>";
      rowContenidoPerfil.appendChild(columna);
    }
  }
}
